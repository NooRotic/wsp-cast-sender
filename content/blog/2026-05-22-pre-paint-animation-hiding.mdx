...
The hero's filter pills (the year-range chips below the hero text) are a CSS `@keyframes` animation, not GSAP. They cascade in with a stagger via a 2.5-second `animation-delay`. During that delay, they need to be invisible. The naive approach:
